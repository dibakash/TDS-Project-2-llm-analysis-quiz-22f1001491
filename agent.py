...
# LLM CONFIGURATION
# -------------------------------------------------
# Choose one of the following models by commenting/uncommenting:

# Option 1: Google Gemini (with rate limiting: 9 requests per minute)
...
# Option 2: OpenAI via aipipe (uncomment to use - rate limiting: 10 requests per minute)
openai_rate_limiter = InMemoryRateLimiter(
    requests_per_second=10 / 60, check_every_n_seconds=1, max_bucket_size=10
)
llm = init_chat_model(
